					...
					<div class="row mb-2">
						<div class="col-sm-8">
							<div class="text-sm-start">
								<a href="/report/insert" class="btn btn-info mb-2">
									등록
								</a>
							</div>
						</div>
						...
						<table id="alternative-page-datatable" class="table table-hover nowrap text-center">
							...
							<tbody>
								...
								<c:choose>
									...
									<c:otherwise>
										<c:forEach var="report" items="${reportList}" varStatus="stat">
											<tr>
												<td>${report.rnum}</td>
												<td class="text-start">
													<a href="/report/detail?reportNo=${report.reportNo}">${report.reportTitle}</a></td>
												<td>${report.reportWriter}</td>
												<td>
													<fmt:formatDate value="${report.reportDate}" pattern="yyyy-MM-dd" /></td>
												<td>${report.reportHit}</td>
											</tr>
										</c:forEach>
									</c:otherwise>
								</c:choose>
